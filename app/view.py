...
import numpy as np
import datetime
from security import AGS_VIEWER, AGS_APPROVER, AGS_SUBMITTER
...
class LRP_View:
    ...
    def view(self):
        if (AGS_SUBMITTER not in st.session_state.user_info["roles"]) and (AGS_APPROVER not in st.session_state.user_info["roles"]) and \
            (AGS_VIEWER not in st.session_state.user_info["roles"]):
            st.title('**:rotating_light: :red[Authorization Unsuccessful] :rotating_light:**')
            st.write(':pushpin: :red[For access, apply to the AGS role:  **_ATTD Assembly Yield Projection LRP - Viewer on AZAD-CORP_**.]')
        else:
            m=st.markdown("""
                            <style>
...
class Data_Entry_View():
        ...
        if st.session_state['RoT_Button']:
            if st.button('**Submit to Database?**'):
                with st.spinner('Writing to Database...'):
                    ...
                    self.de_model.submission.set_Submission_Attributes(field_values_dict=field_values_dict)
    ...
    def view(self):
        if (AGS_SUBMITTER not in st.session_state.user_info["roles"]) and (AGS_APPROVER not in st.session_state.user_info["roles"]):
            st.title('**:rotating_light: :red[Authorization Unsuccessful] :rotating_light:**')
            st.write(':pushpin: :red[Only Yield Analysts part of ATTD Yield organization have access to data entry.]')
...
class Data_Review_View():
    ...
    def create_pending_options(self):
        ...
        for pending_submission in self.dr_model.pending_submissions:
            self.pending_options[f'Submission ID: {pending_submission.submission_id}, ' + \
                                f'User: {pending_submission.user}, ' +  f'Submission Date: {pending_submission.submission_date}'] = pending_submission.submission_id
    ...
    def view(self):
        if AGS_APPROVER not in st.session_state.user_info["roles"]:
            st.title('**:rotating_light: :red[Authorization Unsuccessful] :rotating_light:**')
            st.write(':pushpin: :red[Only members of ATTD Yield Prediction team as part of ATTD Yield organization have access to data review of LRP submissions.]')
        else:
            ...
            if options and st.session_state['get_sub_key']:
                ...
                if st.button(label='**Submit to Database**', key='Review_DB'):
                    if reviewed=='Approve':
                        with st.spinner('Writing to Database...'):
                            record = self.dr_model.create_Approval(field_values_dict=self.dr_model.submission_review.field_values_dict)
                            st.write(record)
...
class Resubmission_View():
    ...
    def create_rejected_options(self):
        self.rs_model.get_Rejected_Submissions()
        for rejected_submission in self.rs_model.rejected_submissions:
            self.rejected_options[f'Submission ID: {rejected_submission.submission_id}, ' + \
                                f'User: {rejected_submission.user}, ' + \
    ...
    def view(self):
        if (AGS_SUBMITTER not in st.session_state.user_info["roles"]) and (AGS_APPROVER not in st.session_state.user_info["roles"]):
            st.title('**:rotating_light: :red[Authorization Unsuccessful] :rotating_light:**')
            st.write(':pushpin: :red[Only Yield Analysts part of ATTD Yield organization have access to resubmission.]')
            st.write(':pushpin: :red[For access, apply to the AGS role:  **_ATTD Assembly Yield Projection LRP - Submitter on AZAD-CORP_**.]')
        else:
            ...
            if options and st.session_state['get_sub_rej_key']:   
                ...
                if 'RoT' in self.rs_model.submission_resubmit.form.form_name:
                    ...
                    with col1:
                        ...
                        create_pqs_input = st.selectbox(label=self.rs_model.submission_resubmit.form.fields['Create_PQS']['Field_Question'], \
                                            options=self.rs_model.submission_resubmit.form.fields['Create_PQS']['Field_Selection'], key='create_pqs_rs_rot', on_change=rerun)
                    ...
                    with col1:
                        ...
                        create_pqs_input = st.selectbox(label=self.rs_model.submission_resubmit.form.fields['Create_PQS']['Field_Question'], \
                                            options=self.rs_model.submission_resubmit.form.fields['Create_PQS']['Field_Selection'], key='create_pqs_rs_me', on_change=rerun)
                    with col2:
                        st.write("**_Other Information_**")
